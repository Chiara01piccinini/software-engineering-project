...
public class CarrelloController {
    ...
    public CarrelloController(Gestore gestore, Marketplace sistema, Session session) {
    ...
    public ResponseEntity<String> rimuoviDalCarrello(@PathVariable UUID id) {
        if (!session.isAuthenticated()) {
        ...
        return risultato
                ? ResponseEntity.ok("Elemento rimosso dal carrello.")
                : ResponseEntity.badRequest().body("Elemento non trovato nel carrello.");
    }
    ...
    public ResponseEntity<String> acquistaCarrello() {
        if (!session.isAuthenticated()) {
