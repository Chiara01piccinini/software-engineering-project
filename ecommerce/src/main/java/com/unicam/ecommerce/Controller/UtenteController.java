...
public class UtenteController {
    ...
    public UtenteController(Marketplace marketplace, Session session, PersistenceManager persistenceManager) {
     ...
     */
    @GetMapping("/prodotti")
    public List<Map<String, Object>> visualizzaProdotti() throws IOException {
     ...
     */
    @GetMapping("/eventi")
    public List<Map<String, Object>> visualizzaEventi() throws IOException {
        List<Map<String, Object>> lista = new ArrayList<>();

        persistenceManager.carica();
        for (Evento p : marketplace.getEventi().values()) {
            Map<String, Object> info = new HashMap<>();
            info.put("tipo", "evento"); // potrebbe essere modificato in "evento" per chiarezza
            info.put("id", p.getId());
            info.put("nome", p.getNome());
