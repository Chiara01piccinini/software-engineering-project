...
public class CuratoreController {
    ...
    public CuratoreController(Session session, Curatore curatore, Gestore gestore, PersistenceManager persistenceManager) {
    ...
    public ResponseEntity<String> approvaProdotto(@PathVariable String id) throws IOException {
    ...
    public ResponseEntity<String> approvaPacchetto(@PathVariable String id) throws IOException {
        if (!session.isAuthenticated()) return ResponseEntity.status(403).body("Devi essere loggato.");
        if (!(session.getCurrentUser().getComponente() instanceof Curatore))
            return ResponseEntity.status(403).body("Solo i curatori possono approvare richieste.");
    ...
    public ResponseEntity<String> approvaEvento(@PathVariable String id) throws IOException {
        if (!session.isAuthenticated()) return ResponseEntity.status(403).body("Devi essere loggato.");
        if (!(session.getCurrentUser().getComponente() instanceof Curatore))
            return ResponseEntity.status(403).body("Solo i curatori possono approvare richieste.");
    ...
    public ResponseEntity<String> approvaInformazioni(@PathVariable String id) throws IOException {
        if (!session.isAuthenticated()) return ResponseEntity.status(403).body("Devi essere loggato.");
        if (!(session.getCurrentUser().getComponente() instanceof Curatore))
            return ResponseEntity.status(403).body("Solo i curatori possono approvare richieste.");
